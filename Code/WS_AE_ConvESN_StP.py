...
steps_ahead = 1
batch_size = 10
train_size = magnitude.shape[0] - (look_back*steps_ahead) - test_size
...
input_data, output_data, data_train_tf, data_test_tf = gen_train_test_data_StP(magnitude, look_back, steps_ahead, test_size, batch_size)
...
with strategy.scope():
    autoencoder = tf.keras.models.load_model(os.getcwd() + f'/models/ae_{nodes_enc[0]}nh_subset_LeakyReLU.keras')
...
with strategy.scope():
    ...
    #declare input
    inputs = Input(shape=(input_data.shape[1:]), name='input')
...
conv_esn.fit(input_data[:train_size], output_data[:train_size],
              epochs=epochs,
              verbose=0,
              callbacks=[reduce_lr]);
...
conv_esn.save(os.getcwd() + f'/models/conv_ESN_{nodes_enc[0]}nh_subset_LeakyReLU_Run{index}_Tau{steps_ahead}_StP.keras')
...
# #get conv_lstm preds
# results_conv_lstm = conv_esn.predict(input_data, verbose=0)
# #output = open(os.getcwd() + f'/models/Conv_LSTM_2.0_predictions_LeakyReLU_Run{index}_Tau{steps_ahead}_StP.pkl', 'wb')
...
predictions = conv_esn.predict(input_data[:-test_size], verbose=0)
last_input = input_data[-test_size,-(look_back-1):,:,:,:]


#recursively forecast remaining test points
results_conv_esn=  np.expand_dims(predictions[-1,:,:,:], axis=0)
new_input = np.concatenate([last_input, np.expand_dims(predictions[-1,:,:,:], axis=0)], axis = 0)
new_input = np.concatenate([new_input[np.newaxis,...]], axis = 0)
...
for j in range(loops):
    ...
    new_input = np.concatenate([new_input[:,-(look_back-1):,:,:,:], np.expand_dims(predictions, axis = 0)], axis = 1)   




#save predictions output
#output = open(os.getcwd() + f'/models/Conv_LSTM_predictions_LeakyReLU_Run{index}_StS{look_fwd}.pkl', 'wb')
#pickle.dump(results_conv_lstm, output)
#output.close()
...
#slice testing points
print()
print(f'Recursive forecasting {steps_ahead}-steps ahead...')
print('Lags Used:', look_back)
print('Steps Ahead:', steps_ahead)
preds = ((windmax-windmin)*results_conv_esn[:,:,:,0] + windmin)
truth = ((windmax-windmin)*windnorm[-test_size:,:,:,0] + windmin)

#calculate mse for each location
print()
print('RMSE of ConvESN (Recursive): {:.6f}'.format(np.sqrt(((truth-preds)**2).mean())))
...
# #add ConvLSTM component --- layer 1
# x, h, c = ConvLSTM2D(x.shape[-1], kernel_size=kernel_size, return_sequences=True, return_state=True, padding='same', name='ConvLSTM1')(x)
# x = LayerNormalization(name='ConvLSTM1_norm')(x)
# h = LayerNormalization(name='ConvLSTM1_hnorm')(h)
# c = LayerNormalization(name='ConvLSTM1_cnorm')(c)


# #add ConvLSTM component --- layer 2
# x = ConvLSTM2D(x.shape[-1], kernel_size=kernel_size, return_sequences=False, padding='same', name='ConvLSTM1Out')(x, initial_state=[h,c])
# x = LeakyReLU(name=f'ConvLSTM2LeakyReLU')(x) #decoder expects data in this format
# x = LayerNormalization(name='ConvLSTM1Out_norm')(x)
